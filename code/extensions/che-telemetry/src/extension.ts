...
export async function activate(context: vscode.ExtensionContext) {
    ...
    telemetryEventService.sendEvent('WORKSPACE_OPENED', context.extensionPath, []);

    context.subscriptions.push(
        vscode.workspace.onDidChangeTextDocument((e: vscode.TextDocumentChangeEvent) => {
            if (e.document.uri.scheme !== 'output') {
                telemetryEventService.sendEvent('EDITOR_USED', context.extensionPath, [['programming language', e.document.languageId]]);
            }
        })
    );
